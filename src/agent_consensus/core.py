...
from typing import Callable, List, Dict, Any, Optional, Sequence, Protocol
...
class Consensus:
    def __init__(
        ...
        judge_fn: Optional[Callable[[list[str]], dict]] = None,
    ) -> None:
        ...
        if strategy == "overlap":
            ...
            def _wrap(cands: Sequence[str]) -> Dict[str, Any]:
                ...
                return {"index": verdict["index"], "rationale": verdict.get("rationale", "")}
    ...
    def pick(self, candidates: Sequence[str]) -> ConsensusResult:
        if not isinstance(candidates, (list, tuple)) or not all(isinstance(x, str) for x in candidates):
            raise TypeError("candidates must be a sequence of str")
        if not candidates:
            return ConsensusResult(index=0, scores=[], rationale="No candidates", extra={})
        ...
        return ConsensusResult(
            ...
            extra={k: v for k, v in out.items() if k not in {"index", "scores", "rationale"}},
        )
